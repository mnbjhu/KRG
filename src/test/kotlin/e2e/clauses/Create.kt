...
class Create: GraphTest() {
    ...
    fun createPath(){
        ...
        graph.query {
            create(::Actor{it[::name] = "John Travolta"} - ::ActedIn{it[::role] = "Vincent Vega"} -
                    ::Movie{ it[::title] = "Pulp Fiction"; it[::releaseYear] = 1994 })
        }
    }
    @Test
    fun createPathToReference(){
        ...
        graph.query {
            ...
            create(::Actor{it[::name] = "John Travolta"} - ::ActedIn{it[::role] = "Vincent Vega"} - pulpFiction)
        }
    }
    @Test
